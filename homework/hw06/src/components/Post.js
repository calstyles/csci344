...
export default function Post({post, token}) { 
    ...
    const [recentComment, setRecentComment] = useState(post.comments[post.comments.length-1]);
    ...
    async function requeryPost(){
        ...
        setRecentComment(data.comments[data.comments.length - 1]);
    }
    ...
    return (
        <div className="card">
          ...
          <AddComment post={post} recentComment={recentComment} requeryPost={requeryPost} token={token} key={"post_" + post.id} />
        </div>
    );
